...
class DaysssVC: UIViewController {
    var days: Array<Day> = []
    var studentCount = 0
    ...
    override func viewDidLoad() {
        ...
        DaysSservice.shared.listenToDays { newDays in
            self.days = newDays
            self.daysTV.reloadData()
        }
        StudentsSservice.shared.listenToStudentCount { newStudentCount in
    ...
    @objc func addDay() {
        let newDayVC = NewDayyyVC()
        present(newDayVC, animated: true, completion: nil)
    }
}


extension DaysssVC: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        let navigationController = UINavigationController(
            rootViewController: DayyyAttendanceVC(dayId: day.id)
        )
